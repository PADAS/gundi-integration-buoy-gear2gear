...
from app.conftest import MockWebhookConfigModel, MockWebhookPayloadModel
...
async def test_process_webhook_request_with_fixed_schema(
    ...
    mocker.patch(
        ...
        AsyncMock(return_value=integration_v2_with_webhook),
    )
    ...
    expected_payload = MockWebhookPayloadModel.parse_obj(
        mock_webhook_request_payload_for_fixed_schema
    )
    expected_config = MockWebhookConfigModel.parse_obj(
        integration_v2_with_webhook.webhook_configuration.data
    )
...
async def test_process_webhook_request_with_dynamic_schema(
    ...
    mocker.patch(
        "app.services.config_manager.IntegrationConfigurationManager.get_integration_details",
        AsyncMock(return_value=integration_v2_with_webhook_generic),
    )
    ...
    expected_config = GenericJsonTransformConfig.parse_obj(
        integration_v2_with_webhook_generic.webhook_configuration.data
    )
    mock_webhook_handler.assert_called_once_with(
        payload=ANY,
...
async def test_get_integration_calls_config_manager_with_ttl(
    ...
    mock_config_manager.get_integration_details = AsyncMock(
    ...
    mock_config_manager.get_integration_details.assert_called_once_with(
        str(integration_v2_with_webhook.id), ttl=60
    )
...
@pytest.mark.asyncio
async def test_get_integration_with_query_param(mocker, integration_v2_with_webhook):
...
async def test_get_integration_handles_config_manager_exception(
    ...
    mock_config_manager = mocker.patch("app.services.webhooks.config_manager")
    mock_config_manager.get_integration_details = AsyncMock(
        side_effect=Exception("Config manager error")
    )
...
async def test_process_webhook_handles_no_integration_gracefully(
    ...
    mocker.patch.object(
        request,
        "json",
        AsyncMock(return_value=mock_webhook_request_payload_for_fixed_schema),
    )
...
async def test_process_webhook_reloads_from_gundi_on_cache_miss(
    ...
    mock_webhook_request_payload_for_fixed_schema,
    mock_redis_empty,
    mock_gundi_client_v2_class_for_webhooks,
    api_client,
):
    ...
    mocker.patch(
        "app.services.config_manager.GundiClient",
        mock_gundi_client_v2_class_for_webhooks,
    )
    ...
    mock_gundi_instance = (
        mock_gundi_client_v2_class_for_webhooks.return_value.__aenter__.return_value
    )
    mock_gundi_instance.get_integration_details.assert_called_with(
...
async def test_process_webhook_handles_gundi_api_failure_gracefully(
    ...
    mock_webhook_request_payload_for_fixed_schema,
    mock_redis_empty,
    mock_gundi_client_v2_class_with_error,
):
    ...
    mocker.patch("app.services.config_manager.redis", mock_redis_empty)
    mocker.patch(
        "app.services.config_manager.GundiClient", mock_gundi_client_v2_class_with_error
    )
    ...
    mock_gundi_instance = (
        mock_gundi_client_v2_class_with_error.return_value.__aenter__.return_value
    )
    mock_gundi_instance.get_integration_details.assert_called_once_with(
        "test-integration-id"
    )
